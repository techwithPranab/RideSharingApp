...
export class PaymentService {
  ...
  static {
    ...
    if (razorpayKeyId && razorpayKeySecret && razorpayKeyId !== 'rzp_test_demo_key' && razorpayKeySecret !== 'demo_secret_key') {
    ...
    if (stripeSecretKey && stripeSecretKey !== 'sk_test_demo_key') {
  ...
  static async createStripePaymentIntent(
    ...
    try {
      if (!this.stripe) {
        throw new Error('Stripe payment gateway not configured. Please configure Stripe keys.');
      }

      // Convert amount to cents/paise for Stripe
  ...
  /**
   * Process ride payment with Stripe
   */
  static async processRidePaymentWithStripe(
    rideId: string,
    payerId: string,
    amount: number,
    currency: string = 'INR'
  ): Promise<{ payment: IPayment; clientSecret: string; paymentIntentId: string }> {
    try {
      // Find the ride
      const ride = await Ride.findById(rideId)
        .populate('passengers.userId')
      ...
      const { clientSecret, paymentIntentId } = await this.createStripePaymentIntent(
        amount,
        currency,
        {
          rideId,
      ...
      logger.error('Error processing ride payment with Stripe:', error);
      throw error;
    }
  }
  ...
  static async handleStripePaymentSuccess(
    ...
    try {
      ...
      logger.info(`Stripe payment completed successfully: ${payment.paymentId}`);
      return payment;
    } catch (error) {
   ...
   */
  static async handleStripePaymentFailure(
    paymentIntentId: string,
    failureReason?: string
  ): Promise<IPayment> {
    try {
      ...
      logger.info(`Stripe payment failed: ${payment.paymentId}, reason: ${failureReason}`);
      return payment;
    } catch (error) {
  ...
  static async processStripeRefund(
    ...
    refundAmount?: number,
    reason?: string
  ): Promise<{ refund: IPayment; stripeRefund: Stripe.Refund }> {
    try {
      // Find original payment
      const originalPayment = await Payment.findOne({ paymentId });
      ...
      const amount = refundAmount || originalPayment.amount;
      const amountInCents = Math.round(amount * 100);
      ...
      logger.info(`Stripe refund processed: ${refund.paymentId} for payment ${paymentId}`);
    ...
    try {
      switch (event.type) {
        case 'payment_intent.succeeded': {
          ...
          await this.handleStripePaymentSuccess(paymentIntent.id, paymentIntent);
          break;
        }
        ...
        case 'charge.dispute.created': {
          ...
          logger.warn(`Payment dispute created: ${dispute.id}`);
          break;
        }
